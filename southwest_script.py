...
async def scrape_destination(page, origin, dest, departure_date, return_day):
    """Search + scrape a single destination. Raises on unrecoverable failure —
    callers are expected to catch per-destination so one bad route (e.g. an
    autocomplete match failure) can't take the whole run down with it."""
    ...
    try:
        ...
        # "networkidle" doesn't reliably fire on this SPA (persistent
        # background traffic keeps the network busy) — wait for the DOM plus
        # the concrete element we need instead of trusting network quiescence.
        await results_page.wait_for_load_state("domcontentloaded")
        ...
        async def wait_for_capture(capture_type, timeout_s=5.0, interval_s=0.1):
            """Poll `captured` for a given response type. Both "hotel" and
            "roomOptions" need this — waiting on "hotel" alone and reading
            "roomOptions" immediately after is what caused room_types to be
            null for ~half of hotels (the roomOptions XHR just hadn't arrived
            yet). If it already arrived, this returns instantly."""
            for _ in range(int(timeout_s / interval_s)):
                if any(t == capture_type for t, _ in captured):
                    return True
        ...
        async def wait_for_result_count(expected_count, timeout_s=8.0, interval_s=0.25):
            """The pagination indicator (aria-current) can flip to the right
            page before every card has actually re-rendered — nth(i) for a
            later card then just times out even though ensure_on_page()
            reported success. Poll for the full card set, not just the first
            one, before resuming the per-hotel loop."""
            view_buttons = results_page.locator(".result-select-button")
            for _ in range(int(timeout_s / interval_s)):
                if await view_buttons.count() >= expected_count:
                    return True
                await results_page.wait_for_timeout(int(interval_s * 1000))
            return await view_buttons.count() >= expected_count
            ...
            for i in range(count):
                ...
                for attempt in range(2):
                    try:
                        ...
                        await wait_for_capture("hotel")
                        await wait_for_capture("roomOptions")
                        ...
                        try:
                            ...
                            elif not await wait_for_result_count(count):
                                print(f"WARNING: only partial result set rendered after resync (page {page_num}, hotel {i})")
                        except Exception:
                            pass

                if not success:
                    print(f"Hotel {i} failed after 2 attempts: {last_error}")
...
async def scope_southwest():
    ...
    async with async_playwright() as p:
        ...
        # background traffic keeps the network busy) — wait for the DOM plus
        # the concrete element we need instead of trusting network quiescence.
        await page.goto("https://www.southwest.com/vacations/", wait_until="domcontentloaded")
        await page.wait_for_selector("#originationAirportCode", timeout=30000)

        possible_destinations = list(dict.fromkeys([
            "Cancun", "Los Cabos", "Punta Cana", "Montego Bay", "Aruba",
            "Turks and Caicos", "St Maarten", "St Thomas", "Grand Cayman",
            "Nassau", 
        ]))
        origin = "BWI"
        ...
        for dest in possible_destinations:
            try:
                await scrape_destination(page, origin, dest, departure_date, return_day)
            except Exception as e:
                print(f"!!! FAILED destination={dest!r} origin={origin!r}: {type(e).__name__}: {e}")
                traceback.print_exc()
